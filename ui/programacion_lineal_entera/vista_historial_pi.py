# ui/programacion_lineal_entera/vista_historial_pi.py
"""
vista_historial_pi.py
=====================
Componente visual de la interfaz de usuario desarrollado sobre Flet (v0.85).
Muestra de forma organizada el histórico de problemas guardados para PLE.
"""
...
def _formatear_objetivo(tipo: TipoOptimizacion, objetivo: List[Fraction]) -> str:
    if not objetivo:
...
@ft.component
def VistaHistorialPi(controlador: ControladorEntera, navegar_a=None):
    refresh_trigger, set_refresh_trigger = ft.use_state(0)
    status_text_val, set_status_text_val = ft.use_state(("", "")) # (message, color)

    historial: List[ProblemaPLE] = controlador.obtener_historial_completo()
    ...
    def manejador_eliminar_problema(indice: int) -> None:
        ...
        set_status_text_val(("✕ El problema seleccionado ha sido eliminado del historial.", RED))
        set_refresh_trigger(lambda x: x + 1)
    ...
    def formatear_restricciones_preview(restricciones: List[Restriccion]) -> str:
        ...
        for r in restricciones[:3]:
            ...
            for i, c in enumerate(r.coeficientes):
                ...
                var = f"X{i+1}"
                if val > 0.0:
                    prefijo = "+ " if terms else ""
                    coef_str = f"{val:.4g}" if val != 1.0 else ""
                    terms.append(f"{prefijo}{coef_str}{var}")
                else:
                    coef_str = f"{abs(val):.4g}" if abs(val) != 1.0 else ""
                    terms.append(f"- {coef_str}{var}")
            terms_str = " ".join(terms) or "0"
            partes.append(f"{terms_str} {r.signo.value} {float(r.rhs):.4g}")
    ...
    def crear_tarjeta_problema(indice: int, problema: ProblemaPLE) -> ft.Container:
        fo_algebraica = _formatear_objetivo(problema.tipo, problema.objetivo)
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Row([
                        ft.Text(f"Problema PI #{indice+1}", size=14, weight=ft.FontWeight.W_600, color=TEXT_PRIMARY),
    ...
    # 1. Cabecera
    header = ft.Column([
        ft.Text("Historial de Problemas PLE", size=20, weight=ft.FontWeight.BOLD, color=TEXT_PRIMARY),
        ft.Text("Consulta, carga, edita o elimina problemas guardados en esta sesión de programación entera.", size=12, color=TEXT_MUTED),
    ], spacing=2)
